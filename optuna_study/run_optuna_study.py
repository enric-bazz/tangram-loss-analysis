...
`<root>/optuna_studies/Dataset3/` where `<root>` is inferred as the parent of
the `data/` folder if present, otherwise as the parent of the provided
dataset folder.

Outputs written to:
  <root>/optuna_studies/DatasetX/{study_name}/{study_name}_best_params.yaml
  <root>/optuna_studies/DatasetX/{study_name}/{study_name}_trials.csv
  <root>/optuna_studies/DatasetX/{study_name}/{study_name}.db   # Optuna sqlite DB (if storage not provided)
"""
...
def infer_output_root(dataset_folder: str) -> str:
    """Infer project root where to place `optuna_studies/DatasetX`.
...
def main(argv: Optional[list] = None):
    ...
    train_genes, val_genes, _ = tgl.split_train_val_test_genes(sc.read_h5ad(sc_path), sc.read_h5ad(st_path), random_state=random_state)

    # infer output root and create output dir
    output_root = infer_output_root(dataset_folder)
    output_dir = os.path.join(output_root, 'optuna_studies', dataset_name, args.study_name)
